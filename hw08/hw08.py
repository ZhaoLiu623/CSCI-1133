...
def grayscale(img_matrix):
    ...
    for y in range(height):
        for x in range (width):
            for i in range(3):
                new_matrix[y][x][i]=img_matrix[y][x][0]*0.3+img_matrix[y][x][1]*0.59+img_matrix[y][x][2]*0.11
                new_matrix[y][x][i]=int(new_matrix[y][x][i])
    return new_matrix
...
def zoom(img_matrix):
    ...
    for y in range(height):
        for x in range (width):
           new_matrix[y][x]= img_matrix[y//2][x//2]
    return new_matrix
